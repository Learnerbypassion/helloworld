...
export default function DoctorDashboard() {
  ...
  const startConsultation = async (qItem) => {
    ...
    // Fetch full session details from backend for real FHIR bundle and documents
    try {
              ...
              {myQueue.map(q => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-500 mt-1">Token: {q.token || `A-${q.id}`} {patient?.abhaId ? `• ABHA: ${patient.abhaId}` : ''}</p>
                  </div>
                );
              })}
            </div>
              ...
              return (
                <>
                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-gray-50">
                    ...
                    {/* Real FHIR Bundle View */}
                    {showFhir && (
                      <div className="bg-gray-900 text-green-400 p-4 rounded-xl font-mono text-xs overflow-x-auto shadow-inner mb-6 max-h-64">
                        <pre>{JSON.stringify(fhirData || {
                          resourceType: "Bundle",
                          type: "transaction",
                          entry: [
                            { resource: { resourceType: "Patient", identifier: [{ value: patient.abhaId || 'ABHA-SYNCED' }] } },
                            { resource: { resourceType: "Condition", code: { text: intake.chiefComplaint?.join(", ") || 'Clinical Case' } } }
                          ]
                        }, null, 2)}</pre>
                      </div>
                    )}
                      ...
                      <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                        ...
                        <div className="space-y-2">
                          {docsToShow.map((doc, i) => {
                            ...
                            const labs = doc.labs || [];
                            return (
                              <div key={i} className="p-3 bg-gray-50 rounded border border-gray-100 space-y-2">
                                ...
                                {(meds.length > 0 || labs.length > 0) && (
                                  <div className="text-xs text-gray-600 pl-8 space-y-1">
                                    {meds.length > 0 && (
                                      <p><strong className="text-brand-800">Meds:</strong> {meds.map(m => m.name || m).join(', ')}</p>
                                    )}
                                    {labs.length > 0 && (
                                      <p><strong className="text-emerald-800">Labs:</strong> {labs.map(l => `${l.test}: ${l.value}`).join(', ')}</p>
                                    )}
                                  </div>
                                )}
                              </div>
